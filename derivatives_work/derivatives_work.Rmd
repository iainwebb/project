...
vector_field_1 <- expand.grid(x1=seq(-5, 5, 1), x2=seq(-5, 5, 1))
...
which has the following slope field and contour plot:

```{r, figures-side, fig.show="hold", out.width="50%", fig.height = 8, echo=F, fig.cap="The three samples of observations, of sizes 11, 31 and 61.", warning=F}
f1 <- expression(
  (100 - x1^2 - x2^2)^(1/2)
)
# D(f1, "x1")
...
The remaining six steps require the selection of another function to compare that from Step 1 to. First we'll look at choices for this second function which offer an acceptable match, initially one which is a perfect match.
